// src/pages/Sales/SalesServiceList.jsx
...
function SalesServiceList() {
  ...
  /* ✅ 세션에서 필터 복원 */
  const savedFilters =
    JSON.parse(sessionStorage.getItem("salesServiceFilters")) || {
      startDate: "",
      endDate: "",
      member: "",
      employee: "",
  ...
  const fetchSalesList = async () => {
    setIsLoading(true);
    try {
      const res = await axios.get("/v1/sales/services/paged", {
        params: {
          page,
          limit: 20,
          startDate: filters.startDate?.trim() ? filters.startDate : undefined,
          endDate: filters.endDate?.trim() ? filters.endDate : undefined,
          serviceNameKeyword: filters.productKeyword?.trim()
      ...
      const updatedList = await Promise.all(
        (list || []).map(async (item) => {
          ...
          if (item.memNum) {
            try {
              const memberRes = await axios.get(`/v1/member/${item.memNum}`);
              updatedItem.memName = memberRes.data.memName || "-";
            } catch {
              updatedItem.memName = "(탈퇴 회원)";
            }
          } else updatedItem.memName = "-";

          if (item.empNum) {
            try {
              const empRes = await axios.get(`/v1/emp/${item.empNum}`);
              updatedItem.empName = empRes.data.empName || "-";
            } catch {
              updatedItem.empName = "(퇴사자)";
            }
          } else updatedItem.empName = "-";
  ...
  const handleFilterChange = (key, value) => {
    ...
    sessionStorage.setItem("salesServiceFilters", JSON.stringify(newFilters));
    setPage(1);
    fetchSalesList();
  };
  ...
  const handleReset = () => {
    ...
    setFilters(reset);
    sessionStorage.setItem("salesServiceFilters", JSON.stringify(reset));
    setPage(1);
    fetchSalesList();
  };
        ...
        >
          ...
          >
            ...
            <div className="d-flex align-items-center flex-shrink-0">
              ...
              <button className="btn btn-outline-secondary">회원 선택</button>
            </div>

            {/* 직원 */}
            <div className="d-flex align-items-center flex-shrink-0">
              <span className="me-2 fw-semibold">직원</span>
              <button className="btn btn-outline-secondary">직원 선택</button>
            </div>
